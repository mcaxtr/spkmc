...
def get_dist_gpu(N, edges, sources, params):
    ...
    if params['distribution'] == 'gamma':
        ...
        times = cp.random.exponential(
            params['lmbd'], size=edges_gpu.shape[0]
        )
    ...
    G.from_cudf_edgelist(df, source='src', destination='dst', edge_attr='weight')  # criação eficiente :contentReference[oaicite:7]{index=7}
    result = cugraph.sssp(G, source=super_node, weight='weight')  # Dijkstra em GPU :contentReference[oaicite:8]{index=8}
...
def multiple_erdos_renyi_gpu(
    num, params, time_steps,
    N=3000, k_avg=10, samples=100,
    initial_perc=0.01, overwrite=False
):
