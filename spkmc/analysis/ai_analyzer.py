...
from spkmc.analysis.prompts import (
    CROSS_EXPERIMENT_SYSTEM_PROMPT,
    SCENARIO_SYSTEM_PROMPT,
    SYSTEM_PROMPT,
    build_collection_prompt,
    build_cross_experiment_prompt,
...
class AIAnalyzer:
    ...
    def analyze_scenario(
        ...
        """
        Generate AI analysis for a single scenario.

        Args:
            scenario_label: Label of the scenario
            result: The loaded result dictionary for this scenario
            results_dir: Path to experiment results directory

        Returns:
            Path to generated analysis file, or None if skipped/failed
        """
        from spkmc.models import Scenario

        normalized = Scenario.normalize_label(scenario_label)
        analysis_path = results_dir / f"{normalized}_analysis.md"
        ...
        # Build prompt
        prompt = build_scenario_prompt(scenario_metrics)
        ...
        # Write analysis file
        with open(analysis_path, "w", encoding="utf-8") as f:
            f.write(f"# Scenario Analysis: {scenario_label}\n\n")
            f.write("---\n\n")
            f.write(str(analysis_text) if analysis_text else "")
            f.write("\n\n---\n\n")
            f.write(f"*Generated by AI analysis (model: {self.model})*\n")
    ...
    def generate_cross_experiment_analysis(
        self,
        ...
        """
        Generate meta-analysis from individual experiment analyses.

        This method creates a synthesis of all individual analysis.md files,
        identifying cross-cutting patterns and unified conclusions.
        ...
        Args:
            ...
            output_path: Path to save the cross-experiment analysis
                (e.g., ./cross_experiment_analysis.md)
        ...
        # Build prompt from individual analyses
        prompt = build_cross_experiment_prompt(experiment_analyses)

        # Call OpenAI API with cross-experiment system prompt
        client = self._get_client()
        response = client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": CROSS_EXPERIMENT_SYSTEM_PROMPT},
                {"role": "user", "content": prompt},
            ],
            temperature=0.3,
            max_tokens=3500,
        ...
        # Write cross-experiment analysis file
        exp_names = [exp["name"] for exp in experiment_analyses]
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("# Cross-Experiment Analysis\n\n")
            f.write(f"**Experiments Analyzed:** {', '.join(exp_names)}\n\n")
            f.write("---\n\n")
            f.write(str(analysis_text) if analysis_text else "")
            f.write("\n\n---\n\n")
            f.write(f"*Generated by AI meta-analysis (model: {self.model})*\n")
